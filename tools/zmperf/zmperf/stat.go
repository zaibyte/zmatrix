...
func (r *Runner) printStat(totalCost, readCost int64) {
	r.printSummary(totalCost)
	r.printIOPS(readCost)
	r.printLat()
}
...
func (r *Runner) printIOPS(readCost int64) {

	getIO := atomic.LoadInt64(&r.getOK)
	getIOFailed := atomic.LoadInt64(&r.getFailed)
